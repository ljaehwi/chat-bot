...
async def lifespan(app: FastAPI):
    ...
    try:
        summarizer_llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # 3. Setup initial state (with db_session)
            ...
            try:
                ...
                async for event in graph.astream_events(initial_state, config=config, version="v1"):
                    ...
                    if kind == "on_chat_model_stream":
                        ...
                        await websocket.send_json({
                            "type": "log",
                            "content": f"🛠️ Calling Tool: {event['name']}({event['data'].get('input')})"
                        })
                    
                    elif kind == "on_tool_end":
                        await websocket.send_json({
                            "type": "log",
                            "content": f"✔️ Tool {event['name']} finished."
                        })
                ...
                if final_state and final_state.values.get("messages"):
                    ...
                    if hasattr(final_message, 'content'):
                        await websocket.send_json({"type": "token", "content": final_message.content})
